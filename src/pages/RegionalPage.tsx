...
import { companies, SECTORS, SIZES } from '../data/mockData'
...
const STATE_REGION: Record<string, string> = {
  SP:'Sudeste', RJ:'Sudeste', MG:'Sudeste', RS:'Sul', PR:'Sul', SC:'Sul',
  BA:'Nordeste', CE:'Nordeste', PE:'Nordeste', GO:'Centro-Oeste', DF:'Centro-Oeste',
  AM:'Norte',
}
...
export default function RegionalPage() {
  ...
  const STATES_LIST = [...new Set(filtered.map((c) => c.state))].sort()
  ...
  const stateData = useMemo(() =>
    STATES_LIST.map((st) => {
      ...
      return {
        ...
        delta,
        empresas: cs.length,
        funcionarios: cs.reduce((s, c) => s + c.employees, 0),
        color: REGION_COLORS[region] || '#009900',
      }
    }).sort((a, b) => b.atual - a.atual),
    [filtered, STATES_LIST],
  )
  ...
  const regionData = useMemo(() => {
    ...
    return regs.map((r) => {
      ...
      return {
        name: r,
        atual:   Math.round(cs.reduce((s, c) => s + c.totalTaxCurrent, 0) / 1000),
        reforma: Math.round(cs.reduce((s, c) => s + c.totalTaxReform, 0)  / 1000),
        receita: Math.round(cs.reduce((s, c) => s + c.revenue, 0)         / 1000),
        empresas: cs.length,
  ...
  // Treemap data for revenue by state
  ...
  const CustomTreemapContent = (props: { x?: number; y?: number; width?: number; height?: number; name?: string; size?: number; color?: string }) => {
      ...
      <div className="card p-4">
        <div className="flex flex-wrap gap-4">
          ...
          <div className="flex items-end">
            <span className="text-sm text-gray-500">{filtered.length} empresas · {STATES_LIST.length} estados</span>
          </div>
        </div>
      </div>
            ...
            <tbody>
              {stateData.map((s, i) => (
                <tr key={s.state} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="py-2.5 px-3 text-right text-gray-700">{fmtM(s.receita * 1000)}</td>
                  <td className="py-2.5 px-3 text-right text-gray-700">{fmtM(s.atual * 1000)}</td>
                  <td className="py-2.5 px-3 text-right text-gray-700">{fmtM(s.reforma * 1000)}</td>
