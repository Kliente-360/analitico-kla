...
const COLUNAS = ['valor_bruto', 'valor_liquido', 'ICMS', 'PIS_COFINS', 'IBS', 'CBS'] as const
...
export default function PBIPage4ProdEnt(): React.ReactElement {
  const rows    = useMemo(() => getPivotPorProduto('Entrada'), [])
  const [filter, setFilter] = useState('')

  const visible = filter
    ? rows.filter((r) => r.descricao_item.toLowerCase().includes(filter.toLowerCase()) || r.categoria.toLowerCase().includes(filter.toLowerCase()))
    : rows
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        <p className="text-xs text-ink-400 flex-1">
          Tabela dinâmica: linhas = produto, colunas = Crédito por ano × métrica.
        </p>
        <input
          type="text"
      ...
      <div className="overflow-x-auto rounded-xl border border-ink-200 shadow-sm">
        <table className="text-[10px] w-full min-w-[1400px]">
          <thead>
            {/* Level 1: ano */}
            <tr className="bg-ink-800 text-white">
              <th className="px-3 py-2 text-left w-48 sticky left-0 bg-ink-800 z-10">Produto</th>
              <th className="px-2 py-2 text-left w-28">Categoria</th>
              {ANOS_REFORMA.map((ano) => (
                <th key={ano} colSpan={COLUNAS.length} className="px-2 py-2 text-center border-l border-ink-600">
                  Crédito {ano}
                </th>
              ))}
            </tr>
            ...
            <tr className="bg-ink-100 border-b border-ink-200">
              <th className="sticky left-0 bg-ink-100 z-10" />
              <th />
              {ANOS_REFORMA.map((ano) =>
                COLUNAS.map((col) => (
                  <th
              ...
              >
                ...
                {ANOS_REFORMA.map((ano) =>
                  COLUNAS.map((col) => {
                    ...
                    const val = col === 'valor_bruto' || col === 'valor_liquido'
                      ? anoData[col]
                      : (anoData[col as 'ICMS' | 'PIS_COFINS' | 'IBS' | 'CBS'] ?? 0)
                    return (
                      <td
                        key={`${ano}-${col}`}
          ...
          {/* Totals row */}
          <tfoot>
            <tr className="bg-ink-100 border-t-2 border-ink-300 font-bold">
              <td className="px-3 py-2 text-ink-800 sticky left-0 bg-ink-100 z-10">TOTAL</td>
              <td />
              {ANOS_REFORMA.map((ano) =>
                COLUNAS.map((col) => {
                  const total = visible.reduce((s, r) => {
                    const d = r.anos[ano]
                    const v = col === 'valor_bruto' || col === 'valor_liquido'
                      ? d[col]
                      : (d[col as 'ICMS' | 'PIS_COFINS' | 'IBS' | 'CBS'] ?? 0)
                    return s + v
                  }, 0)
                  return (
                    <td
